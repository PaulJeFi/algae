struct Entry {
    ...
    int8_t flag;
    Move move;
    Entry(U64 key = 0ULL, int depth = -1, int value = 0, int8_t flag = -1, const Move &move = nullmove);
};
...
class TT {
    ...
    public :
        ...
        void save(const Board &board, uint depth, int8_t flag, int value, uint ply, const Move &move=nullmove, bool timeout=false);
        int hashfull();
};
