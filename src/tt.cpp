...
ProbeEntry::ProbeEntry(const Move &move, int value) {
    this->move = move;
    this->value = value;
}

bool ProbeEntry::is_none() {
    return (move == nullmove) && (value == VALUE_NONE);
}
...
ProbeEntry TT::probe(const Board &board, uint depth, int alpha, int beta, uint ply) {
    ...
    if (board.hash_key == entry.key) {
        ...
        if (depth <= entry.depth) {
            ...
            if ((entry.flag == BOUND_LOWER) && (value >= beta)) {
                return ProbeEntry(entry.move, value);
            } 
            if ((entry.flag == BOUND_UPPER) && (value <= alpha)) {
                return ProbeEntry(entry.move, value);
            }
        }
...
void TT::save(const Board &board, uint depth, int8_t flag, int value, uint ply, const Move &move, bool timeout) {
    ...
    if (!((board.hash_key == entry.key) && (move == nullmove))) {
        entry.move = move;
    }

    Move move_to_store = move;
    if ((board.hash_key == entry.key) && (move == nullmove)) {
        move_to_store = entry.move;
    }
...
int TT::hashfull() {
    ...
    return (int) ceil(1000.0 * counter / size);
}
